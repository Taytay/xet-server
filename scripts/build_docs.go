...
// looking for the top-level go.mod (module github.com/guilt/xet-server), not scripts/go.mod.
...
func repoRootDir() (string, error) {
	...
	for {
		if data, err := os.ReadFile(filepath.Join(dir, "go.mod")); err == nil {
			if strings.HasPrefix(string(data), "module github.com/guilt/xet-server\n") {
				return dir, nil
			}
		}
		parent := filepath.Dir(dir)
		if parent == dir {
			return "", fmt.Errorf("could not find repo root (github.com/guilt/xet-server go.mod) above %s", dir)
		}
		dir = parent
	}
}
...
func genGodoc(repoRoot string) error {
	...
	for _, pkg := range pkgs {
		name := strings.TrimPrefix(pkg, "github.com/guilt/xet-server/")
...
func printMermaidCLIStatus() {
	for _, cli := range mermaidRenderers {
		...
		if mermaidCLIWorks(cli) {
			fmt.Printf("%s [OK] - diagrams will render as SVG\n", cli)
			return
		}
