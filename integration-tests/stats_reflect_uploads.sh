#!/bin/bash
# The /stats endpoint must reflect uploads: file count increases, and unique
# chunk bytes account for dedup (not simply the sum of all uploaded bytes).
set -euo pipefail

statsBefore=$(curl -sf "$XETD_URL/stats")
...
statsAfter=$(curl -sf "$XETD_URL/stats")
