...
```
package hfclient // import "xet-server/internal/hfclient"
...
func (c *Client) Resolve(ctx context.Context, cred auth.CredentialHelper, repoID, revision, filename string) (*ResolveInfo, error)
    Resolve calls HEAD /{repo_id}/resolve/{revision}/{filename} — the real Hub's
    per-file metadata endpoint; huggingface_hub's HEAD call is what triggers the
    Xet download path (see internal/hubserver/resolve.go's doc comment for the
    exact header contract this mirrors).
...
type RepoInfo struct {
    ...
    sha is the one field that actually matters downstream.
